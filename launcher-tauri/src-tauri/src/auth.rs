...
impl DiscordAuth {
    ...
    pub fn authorize_url() -> String {
        format!(
            "{}/appLogin?client_id={}&redirect_uri=amonglauncher%%3A//callback",
            BACKEND_URL, CLIENT_ID,
        )
    }

    pub fn exchange_redirect_uri() -> String {
        "amonglauncher://callback".to_string()
    }
    ...
    pub async fn exchange_token(code: &str) -> Result<TokenResponse, LauncherError> {
        ...
        let resp = client
            ...
            .form(&[
                ...
                ("code", code),
                ("redirect_uri", "amonglauncher://callback"),
            ])
            .send()
            .await
